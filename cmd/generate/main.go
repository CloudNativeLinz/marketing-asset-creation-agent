...
// Usage:
//
//	go run ./cmd/generate -p PROMPT_FILE -i IMAGE [-b BACKGROUND] [-s SIZE]
//
// Required:
//
//	-p  Path to a prompt file (e.g. prompts/speaking-prompt.md)
...
func main() {
	promptFile := flag.String("p", "", "Path to a prompt file (e.g. prompts/speaking-prompt.md) (required)")
	...
	if *promptFile == "" {
		fatalf("Error: Prompt file is required (-p)\nUsage: generate -p PROMPT_FILE -i IMAGE [-b BACKGROUND] [-s SIZE]")
	}
	if *inputImage == "" {
	...
	if prompt == "" {
		fatalf("Error: Prompt file is empty: %s", *promptFile)
	}
